...
- **`STATUS.md` v rootu repo** (pokud existuje) → přečti prvních 80 řádků — extrahuj **aktuální fáze projektu**, **pending tasks**, **post-restart verification kroky**, **known limitations**. Zobraz jako separate panel pokud má hodnotu.
...
Pokud existuje STATUS.md s pending tasks nebo post-restart kroky:

````
╭─ status ─────────────────────────────────────╮
│  fáze:    <last completed> → <next pending>  │
│  ▸ <pending task 1>                          │
│  ▸ <post-restart verification step>          │
╰──────────────────────────────────────────────╯
````
